...
class RouterAddForm(CustomForm):
    ...
    def clean_name(self):
        if self.instance.id:
            router = Router.objects.filter(name__exact=self.cleaned_data['name'], admin_id=_thread_local.user.id).exclude(name=self.cleaned_data['name']).first()
        else:
            router = Router.objects.filter(name__exact=self.cleaned_data['name'], admin_id=_thread_local.user.id).first()

        if router:
            raise forms.ValidationError("This Router is already exist")
        else:
            return self.cleaned_data['name']
    def clean(self):
        cleaned_data = super(RouterAddForm, self).clean()
